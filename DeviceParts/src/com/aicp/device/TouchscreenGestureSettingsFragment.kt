...
class TouchscreenGestureSettingsFragment : PreferenceFragment() {
    ...
    fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        mOffscreenGestureFeedbackSwitch = findPreference(HtcGestureService.Companion.KEY_GESTURE_HAPTIC_FEEDBACK) as TwoStatePreference?
        mOffscreenGestureFeedbackSwitch.setChecked(Settings.System.getInt(getContext().getContentResolver(),
                "Settings.System." + HtcGestureService.Companion.GESTURE_HAPTIC_SETTINGS_VARIABLE_NAME, 1) !== 0)
    }

    fun onPreferenceTreeClick(preference: Preference): Boolean {
        if (preference === mOffscreenGestureFeedbackSwitch) {
            Settings.System.putInt(getContext().getContentResolver(),
                    "Settings.System." + HtcGestureService.Companion.GESTURE_HAPTIC_SETTINGS_VARIABLE_NAME, if (mOffscreenGestureFeedbackSwitch.isChecked()) 1 else 0)
            return true
        }
