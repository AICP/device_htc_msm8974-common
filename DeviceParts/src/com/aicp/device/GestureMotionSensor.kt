...
class GestureMotionSensor private constructor(context: Context) {
    private val mContext: Context
    private val mSensorManager: SensorManager
    private val mSensor: Sensor?
    private var mEnabledGestures: Int = 0
    ...
    fun beginListening() {
        if (!Utils.fileExists(CONTROL_PATH) || !Utils.fileWritable(CONTROL_PATH)) {
            Log.w(TAG, "Control path not accessible, unable to disable sensor")
            return
        }
        Utils.writeValueSimple(CONTROL_PATH, Integer.toHexString(mEnabledGestures))
    ...
    fun stopListening() {
        ...
        Utils.writeValueSimple(CONTROL_PATH, Integer.toHexString(0))
    ...
    private val gestureMotionSensor: Sensor?
         ...
         * isn't returning a valid sensor.
         */
            val it: Iterator<*> = mSensorManager.getSensorList(Sensor.TYPE_ALL).iterator()
            while (it.hasNext()) {
                val sensor: Sensor = it.next() as Sensor
                if (GESTURE_MOTION_SENSOR_NAME == sensor.getName()) {
                    return sensor
                }
            }
    ...
    private val mSensorEventListener: SensorEventListener = object : SensorEventListener {
        override fun onSensorChanged(event: SensorEvent) {
            if (DEBUG) Log.d(TAG, "onSensorChanged: got event: " + event.values.get(0) as Int)
            val sensorEvent = event.values.get(0) as Int
    ...
    companion object {
        ...
        private const val CONTROL_PATH = "/sys/devices/virtual/htc_sensorhub/sensor_hub/gesture_motion"
